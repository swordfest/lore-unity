...
### Fixed
- Reachability check (Test button and status dot) now uses the real `lore` CLI
  transport (`lore repository list lore://host:port`, capped at 8s) instead of
  a raw socket. Both HttpClient AND TcpClient turned out to be unreliable inside
  the Unity editor's Mono networking — reporting reachable servers as offline —
  while the CLI reaches the server correctly.
- Subprocess output is now read asynchronously so process timeouts actually
  fire on a hung connection (a blocking read previously defeated the timeout).
